...
class MigrationManager:
    ...
    def has_json_data(self) -> bool:
        return (
            self.json_file_path.exists()
            and self.json_file_path.stat().st_size > 0
        )
    ...
    def migrate_to_sqlite(self) -> Dict[str, Any]:
        ...
        if self.has_sqlite_data():
            return {
                ...
                "message": "SQLite database already contains data",
            }
        ...
        if not json_data:
            return {
                "status": "empty_json",
                "message": "JSON file is empty or corrupted",
            }
    ...
    def _perform_migration(self, json_data: Dict[str, Any]) -> Dict[str, int]:
        ...
        for session_id, session_data in sessions.items():
            ...
            for collection_id, collection_data in collections.items():
                collection = self._convert_json_collection(
        ...
        if patterns and sessions:
            ...
            latest_session = max(
                sessions.keys(),
                key=lambda x: sessions[x].get("last_updated", ""),
            )
    ...
    def _convert_json_session(
        ...
        return MemorySession(
            ...
            last_updated=session_data.get(
                "last_updated", datetime.now().isoformat()
            ),
        )

    def _convert_json_memory(
    ...
    def _backup_json_file(self):
        ...
        backup_path = self.json_file_path.with_suffix(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Memory Bank MCP Migration Tool"
    )
    parser.add_argument(
        "--project-path", help="Project path (auto-detected if not provided)"
    )
    parser.add_argument(
        "--status", action="store_true", help="Show migration status"
    )
    parser.add_argument(
        "--migrate", action="store_true", help="Perform migration"
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force migration even if SQLite has data",
    )
